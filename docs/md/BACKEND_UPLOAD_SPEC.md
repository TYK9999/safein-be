...
`s3:CompleteMultipartUpload` on the bucket/prefix — plus `s3:GetObject` if the
backend reconciles completion with a `HeadObject` check (recommended: it lets a
retry after a crash mid-completion resolve to the already-finished object and
replay its `videoId`, instead of returning a spurious `EXPIRED_SESSION` for an
upload that actually succeeded). `HeadObject` is authorized by `s3:GetObject` —
there is no separate `s3:HeadObject` action.
...
- [ ] IAM for the presigning role: `s3:CreateMultipartUpload`, `s3:UploadPart`,
      `s3:CompleteMultipartUpload`, and `s3:GetObject` for the `HeadObject`
      completion-recovery check (§4).
