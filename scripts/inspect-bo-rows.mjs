...
const sheets = [...wb.matchAll(/<sheet[^>]+name="([^"]+)"[^>]+r:id="([^"]+)"/g)].map(
  (m) => ({ name: m[1], path: "xl/" + relMap[m[2]].replace(/^\.\//, "") }),
);
...
function getCellValue(cellXml, shared) {
  ...
  if (t === "inlineStr") return cellXml.match(/<t[^>]*>([^<]*)<\/t>/)?.[1] ?? "";
  return v;
}
...
const bo = sheets.find((s) => s.name === "Back Office App");
const grid = readSheet(bo.path);
for (let r = 0; r < Math.min(25, grid.length); r++) {
  const mod = grid[r][0];
  const story = grid[r][2];
  if (mod || story) console.log(`${r + 1}: ${mod} | ${story?.slice(0, 80)}`);
}
